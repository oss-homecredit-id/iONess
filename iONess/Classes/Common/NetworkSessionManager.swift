...
public class NetworkSessionManager {
    ...
    var duplicateRequestHandler: DuplicatedHandler
    
    public init(with session: URLSession, duplicatedHandler: DuplicatedHandler) {
        self.session = session
        self.duplicateRequestHandler = duplicatedHandler
    }
    ...
    public func downloadTask(with request: URLRequest, completionHandler: @escaping URLCompletion<URL>) -> URLSessionDownloadTask {
        var completion: URLCompletion<URL> = completionHandler
        if let prevCompletion: URLCompletion<URL> = currentCompletion(for: request) {
            let decision = duplicateRequestHandler.duplicatedDownload(
                request: request,
    ...
    public func uploadTask(with request: URLRequest, fromFile fileURL: URL, completionHandler: @escaping (Data?, URLResponse?, Error?) -> Void) -> URLSessionUploadTask {
        ...
        if let prevCompletion: URLCompletion<Data> = currentCompletion(for: request) {
            let decision = duplicateRequestHandler.duplicatedUpload(
                request: request,
                withPreviousCompletion: prevCompletion,
                currentCompletion: completionHandler
            )
    ...
    public func dataTask(with request: URLRequest, completionHandler: @escaping (Data?, URLResponse?, Error?) -> Void) -> URLSessionDataTask {
        var completion: URLCompletion<Data> = completionHandler
        if let prevCompletion: URLCompletion<Data> = currentCompletion(for: request) {
            let decision = duplicateRequestHandler.duplicatedData(
                request: request,
                withPreviousCompletion: prevCompletion,
                currentCompletion: completionHandler
            )
